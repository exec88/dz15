...
uint* sha1(char* message, uint msize_bytes) {                  //инициализация
    ...
    uint totalBlockCount = msize_bytes / one_block_size_bytes;                   // подсчет числа блоков
    ...
    *ptr_to_size = bring_to_human_view(msize_bytes * 8);
    ...
    delete[] newMessage;     // чистим за собой
    return digest;
}